...
{
    qDebug()<<"new user come2"<<userName<<localHostName<<ipAddress;
    bool bb = ui->tableWidget->findItems(ipAddress,Qt::MatchExactly).isEmpty();
    if(bb)
    {
...
/*用户离开*/
void Widget::participantLeft(QString userName,QString ipAddress,QString time)
{
    qDebug()<<"leave"<<userName<<ipAddress;
...
void Widget::sendMessage(MessageType type, QString serverAddress)
{
    ...
    switch(type)
    {
        case ParticipantLeft:
            {
                qDebug()<<"case user leave"<<userName;
                break;
            }
        case NewParticipant:
            {         
                out << address;
                qDebug()<<"case new user3";
                break;
            }
